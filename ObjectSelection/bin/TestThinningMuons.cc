...
int main(int argc, char* argv[]) 
{
  ...
  for(unsigned int iFile=0; iFile<inputFiles_.size(); ++iFile){
	...
	//event.getByLabel(std::string("allMuons"), muons);

	edm::Handle<std::vector<Muon> > selectedmuons;
	event.getByLabel(std::string("selectedMuons"), selectedmuons);
	
        //std::cout << "size of original muon collection: " << muons->size() << std::endl;
        std::cout << "size of thinned  muon collection: " << selectedmuons->size() << std::endl;
        std::cout << "--------------------------------------" << std::endl;
	...
	for(std::vector<Muon>::const_iterator mu1=selectedmuons->begin(); mu1!=selectedmuons->end(); ++mu1){
	  ...
	  if( mu1->pt()>20 && fabs(mu1->eta())<2.1 ){
	    for(std::vector<Muon>::const_iterator mu2=selectedmuons->begin(); mu2!=selectedmuons->end(); ++mu2){
