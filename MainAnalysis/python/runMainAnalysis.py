...
if __name__ == "__main__":
    ...
    for edmfile in edmfiles:
        ...
        if "selection" not in edmfile:
            commands.append( "$(echo $CMSSW_BASE)/bin/$(echo $SCRAM_ARCH)/MainAnalysis inputFiles=%s outputFile=%s monitorFile=%s %s" % (edmfile, outputfile, edmfile.split(".root")[0]+"_monitor.root", more ) )
        else:
            commands.append( "$(echo $CMSSW_BASE)/bin/$(echo $SCRAM_ARCH)/MainAnalysis inputFiles=%s outputFile=%s monitorFile=none %s" % (edmfile, outputfile, more ) )
